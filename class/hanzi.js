...
class Hanzi {
    static list = [];
    static bushouListRAW = ["亻","讠","阝","刂","冖","冫","十","八","人","厂","力","卩","廴","勹","厶","匚","冂","彳","女","氵","口","饣","忄","土","扌","子","纟","马","艹","宀","辶","囗","彡","广","夕","寸","大","小","门","巾","山","犭","尸","弓","丬","廾","尢","夂","木","日","王","车","火","礻","攵","灬","心","方","手","止","户","曰","父","爫","月","犬","目","钅","衤","疒","穴","立","田","石","矢","罒","皿","禾","白","鸟","⺮","米","覀","页","舌","缶","耳","虫","虍","舟","足","走","角","身","鱼","隹","雨","齿","革","骨","音"];
    // static bushouList  = [
    ...
    /*
        ...
        王车木礻火灬日曰文月水长戈心气耂父贝见牛牜攵支比爫欠片止户斤手风歹韦瓦毛牙氏旡殳犬斗无冃
        钅衤穴疒白白田矢石母目立龙生鸟癶罒玉禾皿瓜皮矛甘
        衣⺮自糸米覀肉舌页虫舟行耳色羽虍羊而至艮缶齐血臼聿
        足走酉里辛身角豕豆豸辰
        非鱼雨隹青齿采隹
        面音首香鬼革髟
        高
...
const z_section = document.getElementById("z_section");
const switchModeBtnTemp = document.getElementById("switchMode");
const helpButtonTemp = document.getElementById("help_button");
const loadingSection = document.getElementById("loading");
// const header_section = document.getElementById("header");
        ...
        if (rawFile.status === 200 || rawFile.status == 0) {
            ...
            switch (pType) {
                ...
                case "word":
                    ...
                    none(loadingSection);
                    flex(z_section);
                    block(switchModeBtnTemp);
                    block(helpButtonTemp);
                    z_search();
                    break;
                case "example":
    ...
    Z_Word.list.forEach((w) => {
        ...
        Z_Word.expList.forEach((e) => {
            ...
            for (let i = 0; i < e.word.length; i++) {
                ...
                if (e.word.slice(i, cleanedWord.length + i) == cleanedWord) {
                    ...
                    e.word.slice(i, cleanedWord.length + i) +
                    "</span>";
        ...
        if (e.phrase.includes(cleanedWord)) {
            ...
            if (i <= e.phrase.length - cleanedWord.length) {
                ...
                example.phrase +=
                    "<span style='color:red;font-weight:bold;'>" +
                    e.phrase.slice(i, cleanedWord.length + i) +
                    "</span>";
                i += cleanedWord.length - 1;
                } else {
...
// z_section.style.display = "flex";
// switchModeBtnTemp.style.display = "block";
// helpButtonTemp.style.display = "block";
// z_search();